...
export default function StokClient({
  ...
  const totalItemFisik = filteredData.reduce(
    (sum, p) => sum + (p.stok || 0),
    0,
  );
  const totalNilaiAset = filteredData.reduce(
    (sum, p) => sum + (p.hpp || 0) * (p.stok || 0),
    0,
  );
  const totalPotensiPendapatan = filteredData.reduce(
    (sum, p) => sum + (p.price || 0) * (p.stok || 0),
    0,
  );
  ...
  const handleQuickAddSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (quickAddModal.amountToAdd <= 0)
      return showToast("Jumlah harus lebih dari 0", "error");

    setIsQuickAdding(true);
    try {
      await addStockAction(quickAddModal.id, quickAddModal.amountToAdd);
      showToast(
        `Berhasil menambah ${quickAddModal.amountToAdd} stok untuk ${quickAddModal.name}`,
      );
      setQuickAddModal({
        isOpen: false,
        id: "",
  ...
  const executeDelete = async () => {
    try {
      ...
      showToast(
        "Gagal menghapus data. Barang mungkin sedang digunakan di riwayat transaksi.",
        "error",
      );
    } finally {
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 font-medium mt-1">
            Kelola inventaris gudang, harga modal, dan harga jual.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex items-center gap-5 relative overflow-hidden">
          ...
          <div>
            ...
            <h3 className="text-2xl font-black text-[#0a3d4d] mt-1">
              {formatRupiah(totalPotensiPendapatan)}
            </h3>
          </div>
        </div>
      </div>
                    ...
                    >
                      <td className="py-4 px-6 text-center">
                        <input
                          ...
                          checked={isChecked}
                          onChange={(e) =>
                            handleSelectItem(p.id, e.target.checked)
                          }
                          className="w-4 h-4 text-[#29a343] rounded border-gray-300 focus:ring-[#29a343] cursor-pointer"
                        />
                      </td>
                      ...
                      <td className="py-4 px-6">
                        <div className="flex items-center justify-center gap-3">
                          <span
                            className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold tracking-wide ${p.stok <= 5 ? "bg-red-100 text-red-700" : "bg-gray-100 text-gray-700"}`}
                          >
                            {p.stok}
                          </span>
                          <button
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center px-4 bg-[#0a3d4d]/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-300 border border-emerald-100">
            <div className="bg-emerald-50/50 p-6 text-center border-b border-emerald-100 relative">
              <button
                onClick={() =>
                  setQuickAddModal({ ...quickAddModal, isOpen: false })
                }
                className="absolute top-4 right-4 p-2 text-gray-400 hover:text-red-500 bg-white rounded-full transition-colors shadow-sm"
              >
            ...
            <form onSubmit={handleQuickAddSubmit} className="p-6">
              <div className="flex justify-between text-sm font-bold text-gray-500 mb-4 px-2">
                ...
                <span className="text-[#0a3d4d]">
                  {quickAddModal.currentStok} Pcs
                </span>
              </div>
              ...
              <div className="flex items-center bg-gray-50 border border-gray-200 rounded-2xl overflow-hidden mb-6">
                <button
                  ...
                  onClick={() =>
                    setQuickAddModal({
                      ...quickAddModal,
                      amountToAdd: Math.max(1, quickAddModal.amountToAdd - 1),
                    })
                  }
                  className="px-5 py-4 text-gray-500 hover:bg-gray-200 font-black text-lg transition-colors"
                >
                  -
                </button>
                <input
                  ...
                  onChange={(e) =>
                    setQuickAddModal({
                      ...quickAddModal,
                      amountToAdd: parseInt(e.target.value) || 1,
                    })
                  }
                ...
                <button
                  ...
                  onClick={() =>
                    setQuickAddModal({
                      ...quickAddModal,
                      amountToAdd: quickAddModal.amountToAdd + 1,
                    })
                  }
                  className="px-5 py-4 text-gray-500 hover:bg-gray-200 font-black text-lg transition-colors"
                >
              ...
              <button
                ...
                className="w-full bg-[#29a343] hover:bg-[#238a39] text-white py-4 rounded-xl font-black shadow-lg shadow-green-200 transition-all active:scale-95 disabled:opacity-70 flex justify-center items-center"
              >
        ...
        <div className="fixed inset-0 z-[40] flex items-center justify-center px-4 bg-[#0a3d4d]/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-300">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">
                  Sisa Stok <span className="text-red-500">*</span>
                </label>
                <input
                  required
                  type="number"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">
                    Modal (HPP) <span className="text-red-500">*</span>
                  </label>
                  <input
                    required
                    type="number"
              ...
              <div className="p-4 bg-emerald-50 rounded-xl border border-emerald-100 flex justify-between items-center mt-2">
                ...
                <span className="text-lg font-black text-emerald-600">
                  {formatRupiah(Math.max(0, formData.price - formData.hpp))} /
                  Pcs
                </span>
              </div>
            ...
            <div>
              ...
              <p className="text-gray-500 text-sm leading-relaxed">
                {deleteModal.type === "bulk"
                  ? `Anda akan menghapus ${selectedItems.length} barang secara permanen.`
