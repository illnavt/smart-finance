...
import { 
  Search, Download, Wallet, TrendingDown, TrendingUp, 
  ChevronLeft, ChevronRight, Plus, X, Pencil, Trash2, CheckCircle2, ArrowUpRight, ArrowDownRight, FileText
} from "lucide-react";
import { 
...
export default function LaporanClient({ initialData }: { initialData: any[] }) {
  ...
  const [formData, setFormData] = useState({ 
    id: "", tipe: "Pengeluaran", kategori: "", keterangan: "", nominal: 0, 
  ...
  // ==== KALKULASI RINGKASAN ====
  const totalPemasukan = filteredData.filter(i => i.tipe === "Pemasukan").reduce((sum, i) => sum + (i.nominal || 0), 0);
  const totalPengeluaran = filteredData.filter(i => i.tipe === "Pengeluaran").reduce((sum, i) => sum + (i.nominal || 0), 0);
  ...
  const openAddModal = () => {
    setIsEditMode(false);
    setFormData({ id: "", tipe: "Pengeluaran", kategori: "", keterangan: "", nominal: 0, tanggal: new Date().toISOString().split('T')[0] });
    setIsModalOpen(true);
  };
  ...
  const openEditModal = (item: any) => {
    ...
    setFormData({ id: item.id, tipe: item.tipe, kategori: item.kategori, keterangan: item.keterangan || "", nominal: item.nominal, tanggal: formattedDate });
    setIsModalOpen(true);
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.kategori || formData.nominal <= 0) return showToast("Lengkapi data dengan benar!", "error");
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-1000 relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-black text-[#0a3d4d] tracking-tight">Laporan Keuangan</h1>
          <p className="text-gray-500 font-medium mt-1">Pantau arus kas, pemasukan, dan pengeluaran operasional.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex items-center gap-5 relative overflow-hidden">
          ...
          <div><p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Total Pemasukan</p><h3 className="text-2xl font-black text-[#0a3d4d] mt-1">{formatRupiah(totalPemasukan)}</h3></div>
        </div>
        <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex items-center gap-5 relative overflow-hidden">
          <div className="w-14 h-14 rounded-2xl bg-red-50 text-red-600 flex items-center justify-center"><TrendingDown size={28} strokeWidth={2.5} /></div>
          <div><p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Total Pengeluaran</p><h3 className="text-2xl font-black text-[#0a3d4d] mt-1">{formatRupiah(totalPengeluaran)}</h3></div>
        </div>
        <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex items-center gap-5 relative overflow-hidden">
          ...
          <div><p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Saldo Bersih</p><h3 className={`text-2xl font-black mt-1 ${saldoBersih < 0 ? 'text-red-500' : 'text-[#0a3d4d]'}`}>{formatRupiah(saldoBersih)}</h3></div>
        </div>
      </div>

      {/* TABEL KEUANGAN MODERN */}
      <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden">
        <div className="p-6 md:px-8 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <div className="relative w-full sm:w-72">
            ...
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Cari kategori / keterangan..." className="w-full bg-gray-50 border border-gray-200 rounded-xl py-2.5 pl-11 pr-4 text-sm focus:bg-white focus:ring-2 focus:ring-[#29a343]/30 focus:border-[#29a343] transition-all outline-none" />
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50/50">
                ...
                <th className="py-4 px-6 text-xs font-bold text-gray-400 uppercase tracking-widest">Tipe</th>
                <th className="py-4 px-4 text-xs font-bold text-gray-400 uppercase tracking-widest">Kategori & Keterangan</th>
                <th className="py-4 px-6 text-xs font-bold text-gray-400 uppercase tracking-widest">Tanggal</th>
                <th className="py-4 px-8 text-xs font-bold text-gray-400 uppercase tracking-widest text-right">Nominal</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {currentItems.length === 0 ? (
                <tr>
                  <td colSpan={6} className="py-16 text-center text-gray-400">
                    <div className="flex flex-col items-center justify-center gap-3">
                      ...
                      <p className="font-medium">Tidak ada data keuangan yang ditemukan.</p>
                    </div>
                  </td>
                </tr>
              ) : (
                currentItems.map((item) => {
                  ...
                  return (
                  <tr key={item.id} className={`hover:bg-gray-50/50 transition-colors group ${isChecked ? "bg-emerald-50/30" : ""}`}>
                    <td className="py-4 px-6 text-center">
                      <input type="checkbox" checked={isChecked} onChange={(e) => handleSelectItem(item.id, e.target.checked)} className="w-4 h-4 text-[#29a343] rounded border-gray-300 focus:ring-[#29a343] cursor-pointer" />
                    </td>
                    
                    {/* INDIKATOR TIPE PEMASUKAN / PENGELUARAN */}
                    <td className="py-4 px-6">
                      <div className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold tracking-wide ${isPemasukan ? "bg-emerald-100 text-emerald-700 border border-emerald-200" : "bg-red-100 text-red-700 border border-red-200"}`}>
                        {isPemasukan ? <ArrowUpRight size={14} strokeWidth={3} /> : <ArrowDownRight size={14} strokeWidth={3} />}
                        {item.tipe}
                      </div>
                    </td>

                    <td className="py-4 px-4">
                      <p className="font-bold text-[#0a3d4d] text-sm truncate max-w-[250px]">{item.kategori}</p>
                      <p className="text-xs text-gray-500 font-medium truncate max-w-[250px]">{item.keterangan || "-"}</p>
                    </td>
                    <td className="py-4 px-6"><p className="text-sm text-gray-500 font-medium whitespace-nowrap">{mounted ? formatDate(item.tanggal) : "..."}</p></td>
                    
                    {/* NOMINAL DENGAN WARNA */}
                    <td className="py-4 px-8 text-right">
                      <p className={`text-sm font-black ${isPemasukan ? "text-emerald-600" : "text-[#0a3d4d]"}`}>
                    ...
                    <td className="py-4 px-6 text-center">
                      <div className="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => openEditModal(item)} className="p-2 text-blue-500 hover:bg-blue-50 rounded-lg transition-colors" title="Edit"><Pencil size={18} /></button>
                        <button onClick={() => setDeleteModal({ isOpen: true, type: "single", data: item.id })} className="p-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors" title="Hapus"><Trash2 size={18} /></button>
                      </div>
                    </td>
                  </tr>
                  );
        ...
        <div className="fixed inset-0 z-[40] flex items-center justify-center px-4 bg-[#0a3d4d]/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="flex items-center justify-between p-6 border-b border-gray-100">
              <h3 className="text-xl font-black text-[#0a3d4d]">{isEditMode ? "Edit Catatan" : "Tambah Catatan Kas"}</h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">Tipe Arus Kas <span className="text-red-500">*</span></label>
                  <select required value={formData.tipe} onChange={(e) => setFormData({...formData, tipe: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:bg-white focus:ring-2 focus:ring-[#29a343]/30 focus:border-[#29a343] outline-none cursor-pointer">
                    <option value="Pengeluaran">Pengeluaran</option>
                    <option value="Pemasukan">Pemasukan</option>
                  </select>
                </div>
                <div>
                  ...
                  <input required type="date" value={formData.tanggal} onChange={(e) => setFormData({...formData, tanggal: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:bg-white focus:ring-2 focus:ring-[#29a343]/30 focus:border-[#29a343] outline-none" />
                </div>
              </div>

              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">Kategori <span className="text-red-500">*</span></label>
                <input required type="text" value={formData.kategori} onChange={(e) => setFormData({...formData, kategori: e.target.value})} placeholder="Contoh: Sewa Tempat, Gaji Karyawan" className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:bg-white focus:ring-2 focus:ring-[#29a343]/30 focus:border-[#29a343] outline-none" />
              </div>

              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">Nominal (Rp) <span className="text-red-500">*</span></label>
                <input required type="number" min="1" value={formData.nominal || ""} onChange={(e) => setFormData({...formData, nominal: parseInt(e.target.value) || 0})} placeholder="Masukkan jumlah..." className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:bg-white focus:ring-2 focus:ring-[#29a343]/30 focus:border-[#29a343] outline-none font-bold text-[#0a3d4d]" />
              </div>

              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">Keterangan Tambahan <span className="text-gray-400 font-normal">(Opsional)</span></label>
                <textarea rows={2} value={formData.keterangan} onChange={(e) => setFormData({...formData, keterangan: e.target.value})} placeholder="Rincian pembayaran..." className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:bg-white focus:ring-2 focus:ring-[#29a343]/30 focus:border-[#29a343] outline-none resize-none"></textarea>
              </div>
        ...
        <div className="fixed inset-0 z-[50] flex items-center justify-center px-4 bg-[#0a3d4d]/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-300 p-8 text-center space-y-6">
            ...
            <div>
              ...
              <p className="text-gray-500 text-sm leading-relaxed">{deleteModal.type === "bulk" ? `Anda akan menghapus ${selectedItems.length} catatan keuangan secara permanen.` : "Anda akan menghapus catatan ini secara permanen."}</p>
            </div>
